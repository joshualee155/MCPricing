...
void FDAmericanBSEngine::calculate() const
{
    ...
    for (int i = 0; i < nS+1; ++i)
    {
        u_back[i] = type == VanillaPayoff::Call ? max(S_mesh[i] - Strike, 0.0) : max(Strike - S_mesh[i], 0.0);
    }
    ...
    for (int j = static_cast<int>(nT) - 1; j >= 0; --j)
    {
        ...
        if (T_mesh[j] > Time0)
        {
            for (int i = 0; i < nS + 1; ++i)
            {
                u_front[i] = max(u_front[i],type == VanillaPayoff::Call ? max(S_mesh[i]*exp(-mu*(Time - T_mesh[j])) - Strike, 0.0) : max(Strike - S_mesh[i]*exp(-mu*(Time - T_mesh[j])), 0.0));
            }
        }
        u_back = u_front;
    }
